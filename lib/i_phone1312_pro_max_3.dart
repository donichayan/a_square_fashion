import 'package:flutter/material.dart';
import 'package:adobe_xd/pinned.dart';
import 'package:flutter_svg/flutter_svg.dart';

class IPhone1312ProMax3 extends StatelessWidget {
  const IPhone1312ProMax3({Key? key}) : super(key: key);
  @override
  Widget build(BuildContext context) {
    return Scaffold(
      backgroundColor: const Color(0xffffffff),
      body: Stack(
        children: <Widget>[
          Pinned.fromPins(
            Pin(start: 0.0, end: 0.0),
            Pin(size: 1134.0, start: 0.0),
            child: Container(
              decoration: BoxDecoration(
                color: const Color(0xff373636),
                border: Border.all(width: 1.0, color: const Color(0xff707070)),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(start: 0.0, end: 0.0),
            Pin(size: 749.0, start: 0.0),
            child: Container(
              decoration: BoxDecoration(
                gradient: const LinearGradient(
                  begin: Alignment(-0.761, -0.835),
                  end: Alignment(0.126, 0.084),
                  colors: [Color(0xfff3e08d), Color(0xffe4be63)],
                  stops: [0.0, 1.0],
                ),
                border: Border.all(width: 1.0, color: const Color(0xff707070)),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(start: 17.0, end: 13.0),
            Pin(size: 19.0, start: 10.0),
            child: Stack(
              children: <Widget>[
                Align(
                  alignment: const Alignment(1.0, 0.483),
                  child: SizedBox(
                    width: 68.0,
                    height: 13.0,
                    child: Stack(
                      children: <Widget>[
                        Container(),
                        Stack(
                          children: <Widget>[
                            Pinned.fromPins(
                              Pin(size: 24.5, end: 1.0),
                              Pin(start: 0.6, end: 0.9),
                              child: Stack(
                                children: <Widget>[
                                  SizedBox.expand(
                                      child: SvgPicture.string(
                                    _svg_ipnvfy,
                                    allowDrawingOutsideViewBox: true,
                                    fit: BoxFit.fill,
                                  )),
                                  Pinned.fromPins(
                                    Pin(size: 18.0, end: 4.5),
                                    Pin(size: 7.7, middle: 0.5),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        color: const Color(0xff323f4b),
                                        borderRadius:
                                            BorderRadius.circular(1.6),
                                      ),
                                    ),
                                  ),
                                ],
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 17.1, start: 0.0),
                              Pin(start: 1.0, end: 1.3),
                              child: SvgPicture.string(
                                _svg_hpzmte,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Pinned.fromPins(
                              Pin(size: 15.4, middle: 0.4202),
                              Pin(start: 0.8, end: 1.1),
                              child: SvgPicture.string(
                                _svg_vql4yc,
                                allowDrawingOutsideViewBox: true,
                                fit: BoxFit.fill,
                              ),
                            ),
                            Container(),
                          ],
                        ),
                      ],
                    ),
                  ),
                ),
                Align(
                  alignment: Alignment.topLeft,
                  child: SizedBox(
                    width: 34.0,
                    height: 19.0,
                    child: Stack(
                      children: const <Widget>[
                        Align(
                          alignment: Alignment.topLeft,
                          child: SizedBox(
                            width: 34.0,
                            height: 19.0,
                            child: Text(
                              '19:27',
                              style: TextStyle(
                                fontFamily: 'Montserrat',
                                fontSize: 15,
                                color: Color(0xff323f4b),
                                letterSpacing: -0.16499999999999998,
                              ),
                              softWrap: false,
                            ),
                          ),
                        ),
                      ],
                    ),
                  ),
                ),
              ],
            ),
          ),
          Pinned.fromPins(
            Pin(size: 52.0, start: 17.0),
            Pin(size: 52.0, start: 45.0),
            child: Stack(
              children: <Widget>[
                Stack(
                  children: <Widget>[
                    Padding(
                      padding: const EdgeInsets.fromLTRB(
                          -278.5, -53.4, -169.5, -228.1),
                      child: Stack(
                        children: <Widget>[
                          Stack(
                            children: <Widget>[
                              Container(
                                decoration: const BoxDecoration(
                                  image: DecorationImage(
                                    image: AssetImage(''),
                                    fit: BoxFit.fill,
                                  ),
                                ),
                              ),
                              Container(
                                decoration: const BoxDecoration(),
                              ),
                            ],
                          ),
                        ],
                      ),
                    ),
                    Container(
                      decoration: const BoxDecoration(
                        borderRadius: BorderRadius.only(
                          topLeft: Radius.circular(17.1),
                          topRight: Radius.circular(17.1),
                          bottomRight: Radius.circular(17.1),
                          bottomLeft: Radius.circular(17.11),
                        ),
                      ),
                    ),
                  ],
                ),
              ],
            ),
          ),
          Pinned.fromPins(
            Pin(size: 21.9, end: 20.1),
            Pin(size: 28.0, start: 55.5),
            child: Stack(
              children: <Widget>[
                Stack(
                  children: <Widget>[
                    Pinned.fromPins(
                      Pin(size: 18.5, start: 0.0),
                      Pin(start: 6.3, end: 5.7),
                      child: SvgPicture.string(
                        _svg_he76qv,
                        allowDrawingOutsideViewBox: true,
                        fit: BoxFit.fill,
                      ),
                    ),
                    Align(
                      alignment: Alignment(-0.198, 1.0),
                      child: SizedBox(
                        width: 4.0,
                        height: 3.0,
                        child: SvgPicture.string(
                          _svg_oq9a97,
                          allowDrawingOutsideViewBox: true,
                        ),
                      ),
                    ),
                    Align(
                      alignment: Alignment.topRight,
                      child: SizedBox(
                        width: 13.0,
                        height: 13.0,
                        child: SvgPicture.string(
                          _svg_h0g8bd,
                          allowDrawingOutsideViewBox: true,
                        ),
                      ),
                    ),
                    Container(
                      decoration: const BoxDecoration(),
                    ),
                  ],
                ),
              ],
            ),
          ),
          Pinned.fromPins(
            Pin(start: 17.0, end: 13.0),
            Pin(size: 83.0, start: 129.0),
            child: Container(
              decoration: BoxDecoration(
                color: const Color(0xff373636),
                borderRadius: BorderRadius.circular(16.0),
                border: Border.all(width: 1.0, color: const Color(0xff45250f)),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(size: 161.0, start: 38.5),
            Pin(size: 24.0, start: 146.0),
            child: const Text(
              'GOLD MEMBER',
              style: TextStyle(
                fontFamily: 'Montserrat-SemiBold',
                fontSize: 20,
                color: Color(0xffe4be63),
                height: 1.2,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 129.0, start: 38.5),
            Pin(size: 24.0, start: 171.0),
            child: const Text.rich(
              TextSpan(
                style: TextStyle(
                  fontFamily: 'Montserrat-Light',
                  fontSize: 20,
                  color: Color(0xffe4be63),
                  height: 1.2,
                ),
                children: [
                  TextSpan(
                    text: '12',
                  ),
                  TextSpan(
                    text: '3',
                    style: TextStyle(
                      letterSpacing: -0.1,
                    ),
                  ),
                  TextSpan(
                    text: '4',
                    style: TextStyle(
                      letterSpacing: -0.32,
                    ),
                  ),
                  TextSpan(
                    text: '-',
                    style: TextStyle(
                      letterSpacing: 0.18,
                    ),
                  ),
                  TextSpan(
                    text: '56',
                  ),
                  TextSpan(
                    text: '7',
                    style: TextStyle(
                      letterSpacing: -0.4,
                    ),
                  ),
                  TextSpan(
                    text: '-',
                    style: TextStyle(
                      letterSpacing: -0.18,
                    ),
                  ),
                  TextSpan(
                    text: '890',
                  ),
                ],
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 48.0, end: 26.0),
            Pin(size: 48.0, start: 146.0),
            child: Stack(
              children: <Widget>[
                Container(
                  decoration: BoxDecoration(
                    color: const Color(0xffe4be63),
                    borderRadius: BorderRadius.circular(14.0),
                  ),
                ),
                Center(
                  child: SizedBox(
                    width: 15.0,
                    height: 18.0,
                    child: Stack(
                      children: <Widget>[
                        Pinned.fromPins(
                          Pin(size: 5.7, start: 0.0),
                          Pin(size: 1.0, start: 2.5),
                          child: SvgPicture.string(
                            _svg_cuxc0u,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          ),
                        ),
                        Pinned.fromPins(
                          Pin(size: 5.7, end: 0.0),
                          Pin(size: 1.0, start: 2.5),
                          child: SvgPicture.string(
                            _svg_t8k8zx,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          ),
                        ),
                        Align(
                          alignment: Alignment.centerLeft,
                          child: SizedBox(
                            width: 7.0,
                            height: 1.0,
                            child: SvgPicture.string(
                              _svg_ppjs0r,
                              allowDrawingOutsideViewBox: true,
                            ),
                          ),
                        ),
                        Align(
                          alignment: Alignment.centerRight,
                          child: SizedBox(
                            width: 4.0,
                            height: 1.0,
                            child: SvgPicture.string(
                              _svg_hia3z,
                              allowDrawingOutsideViewBox: true,
                            ),
                          ),
                        ),
                        Pinned.fromPins(
                          Pin(size: 4.1, start: 0.0),
                          Pin(size: 1.0, end: 1.5),
                          child: SvgPicture.string(
                            _svg_r226f3,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          ),
                        ),
                        Pinned.fromPins(
                          Pin(size: 7.4, end: 0.0),
                          Pin(size: 1.0, end: 1.5),
                          child: SvgPicture.string(
                            _svg_u7ts9w,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          ),
                        ),
                        Align(
                          alignment: const Alignment(-0.166, -1.0),
                          child: SizedBox(
                            width: 1.0,
                            height: 5.0,
                            child: SvgPicture.string(
                              _svg_ighjoy,
                              allowDrawingOutsideViewBox: true,
                            ),
                          ),
                        ),
                        Align(
                          alignment: const Alignment(0.55, 0.0),
                          child: SizedBox(
                            width: 1.0,
                            height: 5.0,
                            child: SvgPicture.string(
                              _svg_d251xk,
                              allowDrawingOutsideViewBox: true,
                            ),
                          ),
                        ),
                        Align(
                          alignment: const Alignment(-0.404, 1.0),
                          child: SizedBox(
                            width: 1.0,
                            height: 5.0,
                            child: SvgPicture.string(
                              _svg_c8w2ub,
                              allowDrawingOutsideViewBox: true,
                            ),
                          ),
                        ),
                      ],
                    ),
                  ),
                ),
              ],
            ),
          ),
          Pinned.fromPins(
            Pin(size: 190.0, middle: 0.3697),
            Pin(size: 48.0, start: 48.0),
            child: const Text.rich(
              TextSpan(
                style: TextStyle(
                  fontFamily: 'Montserrat',
                  fontSize: 20,
                  color: Color(0xff161615),
                  height: 1.2,
                ),
                children: [
                  TextSpan(
                    text: 'Welcome \n',
                    style: TextStyle(
                      fontWeight: FontWeight.w100,
                    ),
                  ),
                  TextSpan(
                    text: 'to Vie Fashion App',
                    style: TextStyle(
                      fontWeight: FontWeight.w500,
                    ),
                  ),
                ],
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 46.0, end: 13.0),
            Pin(size: 20.0, start: 231.0),
            child: const Text(
              'See all',
              style: TextStyle(
                fontFamily: 'Poppins',
                fontSize: 14,
                color: Color(0xffffffff),
                fontWeight: FontWeight.w600,
                height: 3.7142857142857144,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 161.0, start: 18.0),
            Pin(size: 25.0, start: 228.0),
            child: const Text(
              'Upcoming Events',
              style: TextStyle(
                fontFamily: 'Poppins',
                fontSize: 18,
                color: const Color(0xff080a24),
                fontWeight: FontWeight.w700,
                height: 2.888888888888889,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 182.0, start: 18.0),
            Pin(size: 24.0, start: 533.0),
            child: Text(
              'Designers / Brands',
              style: TextStyle(
                fontFamily: 'Montserrat-SemiBold',
                fontSize: 20,
                color: const Color(0xff08080a),
                letterSpacing: -0.8,
                height: 1.85,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 236.0, start: 145.0),
            Pin(size: 24.0, start: 795.0),
            child: Text(
              'Vie International Awards',
              style: TextStyle(
                fontFamily: 'Montserrat-SemiBold',
                fontSize: 20,
                color: const Color(0xffffffff),
                letterSpacing: -0.8,
                height: 1.85,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(start: -0.6, end: 0.0),
            Pin(size: 203.0, middle: 0.2864),
            child: SingleChildScrollView(
              primary: false,
              scrollDirection: Axis.horizontal,
              child: SizedBox(
                width: 1129.0,
                height: 203.0,
                child: Stack(
                  children: <Widget>[
                    Padding(
                      padding: EdgeInsets.fromLTRB(0.0, 0.0, -700.0, 0.0),
                      child: SingleChildScrollView(
                        primary: false,
                        child: Wrap(
                          alignment: WrapAlignment.center,
                          spacing: 168,
                          runSpacing: 0,
                          children: [
                            {
                              'text': 'John Rascal Doe',
                            },
                            {
                              'text': 'John Doe',
                            }
                          ].map((itemData) {
                            final text = itemData['text'];
                            return SizedBox(
                              width: 301.0,
                              height: 148.0,
                              child: Stack(
                                children: <Widget>[
                                  Pinned.fromPins(
                                    Pin(size: 143.6, start: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Stack(
                                      children: <Widget>[
                                        Align(
                                          alignment: Alignment.topLeft,
                                          child: SizedBox(
                                            width: 144.0,
                                            height: 148.0,
                                            child: Stack(
                                              children: <Widget>[
                                                Container(
                                                  decoration: BoxDecoration(
                                                    color:
                                                        const Color(0xffffffff),
                                                    borderRadius:
                                                        BorderRadius.circular(
                                                            12.0),
                                                    boxShadow: const [
                                                      BoxShadow(
                                                        color:
                                                            Color(0x24173a6e),
                                                        offset: Offset(0, 3),
                                                        blurRadius: 36,
                                                      ),
                                                    ],
                                                  ),
                                                ),
                                                Container(
                                                  decoration: BoxDecoration(
                                                    borderRadius:
                                                        BorderRadius.circular(
                                                            12.0),
                                                    boxShadow: const [
                                                      BoxShadow(
                                                        color:
                                                            Color(0x08141267),
                                                        offset: Offset(0, 29),
                                                        blurRadius: 23,
                                                      ),
                                                    ],
                                                  ),
                                                ),
                                              ],
                                            ),
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(start: 5.4, end: 5.4),
                                          Pin(size: 96.9, start: 5.0),
                                          child: Stack(
                                            children: <Widget>[
                                              Container(
                                                decoration: const BoxDecoration(
                                                  image: DecorationImage(
                                                    image: AssetImage(''),
                                                    fit: BoxFit.fill,
                                                  ),
                                                ),
                                                margin:
                                                    const EdgeInsets.fromLTRB(
                                                        -30.0,
                                                        -6.0,
                                                        -37.2,
                                                        -30.1),
                                              ),
                                              Align(
                                                alignment: Alignment.topLeft,
                                                child: Container(
                                                  width: 133.0,
                                                  height: 97.0,
                                                  decoration: BoxDecoration(
                                                    color:
                                                        const Color(0xfff3d1ab),
                                                    borderRadius:
                                                        BorderRadius.circular(
                                                            11.0),
                                                  ),
                                                ),
                                              ),
                                            ],
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(size: 114.0, start: 5.6),
                                          Pin(size: 18.0, middle: 0.8151),
                                          child: Stack(
                                            children: <Widget>[
                                              Align(
                                                alignment: Alignment.topLeft,
                                                child: SizedBox(
                                                  width: 114.0,
                                                  height: 18.0,
                                                  child: Text(
                                                    text!,
                                                    style: const TextStyle(
                                                      fontFamily: 'Montserrat',
                                                      fontSize: 14,
                                                      color: Color(0xff08080a),
                                                      letterSpacing: -0.14,
                                                      height:
                                                          1.0714285714285714,
                                                    ),
                                                    textHeightBehavior:
                                                        const TextHeightBehavior(
                                                            applyHeightToFirstAscent:
                                                                false),
                                                    softWrap: false,
                                                  ),
                                                ),
                                              ),
                                            ],
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 143.6, end: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Stack(
                                      children: <Widget>[
                                        Align(
                                          alignment: Alignment.topLeft,
                                          child: SizedBox(
                                            width: 144.0,
                                            height: 148.0,
                                            child: Stack(
                                              children: <Widget>[
                                                Container(
                                                  decoration: BoxDecoration(
                                                    color:
                                                        const Color(0xffffffff),
                                                    borderRadius:
                                                        BorderRadius.circular(
                                                            12.0),
                                                    boxShadow: const [
                                                      BoxShadow(
                                                        color:
                                                            Color(0x24173a6e),
                                                        offset: Offset(0, 3),
                                                        blurRadius: 36,
                                                      ),
                                                    ],
                                                  ),
                                                ),
                                                Container(
                                                  decoration: BoxDecoration(
                                                    borderRadius:
                                                        BorderRadius.circular(
                                                            12.0),
                                                    boxShadow: const [
                                                      BoxShadow(
                                                        color:
                                                            Color(0x08141267),
                                                        offset: Offset(0, 29),
                                                        blurRadius: 23,
                                                      ),
                                                    ],
                                                  ),
                                                ),
                                              ],
                                            ),
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(start: 5.4, end: 5.4),
                                          Pin(size: 96.9, start: 5.0),
                                          child: Stack(
                                            children: <Widget>[
                                              Container(
                                                decoration: const BoxDecoration(
                                                  image: DecorationImage(
                                                    image: AssetImage(''),
                                                    fit: BoxFit.fill,
                                                  ),
                                                ),
                                                margin:
                                                    const EdgeInsets.fromLTRB(
                                                        -55.6,
                                                        -7.0,
                                                        -55.6,
                                                        -33.1),
                                              ),
                                              Align(
                                                alignment: Alignment.topLeft,
                                                child: Container(
                                                  width: 133.0,
                                                  height: 97.0,
                                                  decoration: BoxDecoration(
                                                    color:
                                                        const Color(0xfff3d1ab),
                                                    borderRadius:
                                                        BorderRadius.circular(
                                                            8.0),
                                                  ),
                                                ),
                                              ),
                                            ],
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(size: 66.0, start: 5.6),
                                          Pin(size: 18.0, middle: 0.8151),
                                          child: Stack(
                                            children: <Widget>[
                                              Align(
                                                alignment: Alignment.topLeft,
                                                child: SizedBox(
                                                  width: 66.0,
                                                  height: 18.0,
                                                  child: Text(
                                                    'John Doe',
                                                    style: TextStyle(
                                                      fontFamily: 'Montserrat',
                                                      fontSize: 14,
                                                      color: const Color(
                                                          0xff08080a),
                                                      letterSpacing: -0.14,
                                                      height:
                                                          2.2857142857142856,
                                                    ),
                                                    textHeightBehavior:
                                                        TextHeightBehavior(
                                                            applyHeightToFirstAscent:
                                                                false),
                                                    softWrap: false,
                                                  ),
                                                ),
                                              ),
                                            ],
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                            );
                          }).toList(),
                        ),
                      ),
                    ),
                  ],
                ),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(size: 48.0, start: 367.0),
            Pin(size: 18.0, start: 537.0),
            child: Text(
              'See All',
              style: TextStyle(
                fontFamily: 'Montserrat-SemiBold',
                fontSize: 14,
                color: const Color(0xffffffff),
                letterSpacing: -0.14,
                height: 2.2857142857142856,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 163.0, start: -11.0),
            Pin(size: 320.0, middle: 0.4599),
            child: Container(
              decoration: BoxDecoration(
                image: DecorationImage(
                  image: const AssetImage(''),
                  fit: BoxFit.fill,
                ),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(size: 260.0, end: 23.0),
            Pin(size: 233.0, middle: 0.4677),
            child: Stack(
              children: <Widget>[
                Align(
                  alignment: Alignment.topLeft,
                  child: SizedBox(
                    width: 260.0,
                    height: 233.0,
                    child: Stack(
                      children: <Widget>[
                        Container(
                          decoration: BoxDecoration(
                            color: const Color(0xffffffff),
                            borderRadius: BorderRadius.circular(12.0),
                            boxShadow: [
                              BoxShadow(
                                color: const Color(0x24173a6e),
                                offset: Offset(0, 3),
                                blurRadius: 36,
                              ),
                            ],
                          ),
                        ),
                        Container(
                          decoration: BoxDecoration(
                            borderRadius: BorderRadius.circular(12.0),
                            boxShadow: [
                              BoxShadow(
                                color: const Color(0x08141267),
                                offset: Offset(0, 29),
                                blurRadius: 23,
                              ),
                            ],
                          ),
                        ),
                      ],
                    ),
                  ),
                ),
                Pinned.fromPins(
                  Pin(start: 10.0, end: 9.0),
                  Pin(size: 176.0, start: 12.0),
                  child: Stack(
                    children: <Widget>[
                      Container(
                        decoration: BoxDecoration(
                          image: DecorationImage(
                            image: const AssetImage(''),
                            fit: BoxFit.fill,
                          ),
                        ),
                        margin: EdgeInsets.fromLTRB(-93.0, -41.0, -21.0, -18.0),
                      ),
                      Align(
                        alignment: Alignment.topLeft,
                        child: Container(
                          width: 241.0,
                          height: 176.0,
                          decoration: BoxDecoration(
                            color: const Color(0xfff3d1ab),
                            borderRadius: BorderRadius.circular(12.0),
                          ),
                        ),
                      ),
                    ],
                  ),
                ),
                Pinned.fromPins(
                  Pin(size: 94.0, start: 12.0),
                  Pin(size: 16.0, start: 201.0),
                  child: Text(
                    'View Winners',
                    style: TextStyle(
                      fontFamily: 'Montserrat-Bold',
                      fontSize: 13,
                      color: const Color(0xff08080a),
                      letterSpacing: -0.13,
                      height: 2,
                    ),
                    textHeightBehavior:
                        TextHeightBehavior(applyHeightToFirstAscent: false),
                    softWrap: false,
                  ),
                ),
              ],
            ),
          ),
          Pinned.fromPins(
            Pin(start: 0.0, end: 0.0),
            Pin(size: 172.0, middle: 0.7254),
            child: Container(
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  begin: Alignment(-0.761, -0.835),
                  end: Alignment(0.126, 0.084),
                  colors: [const Color(0xfff3e08d), const Color(0xffe4be63)],
                  stops: [0.0, 1.0],
                ),
                border: Border.all(width: 1.0, color: const Color(0xff707070)),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(start: 0.0, end: 0.0),
            Pin(size: 6.0, middle: 0.5606),
            child: Container(
              decoration: BoxDecoration(
                gradient: LinearGradient(
                  begin: Alignment(-0.761, -0.835),
                  end: Alignment(0.126, 0.084),
                  colors: [const Color(0xfff3e08d), const Color(0xffe4be63)],
                  stops: [0.0, 1.0],
                ),
                border: Border.all(width: 1.0, color: const Color(0xff707070)),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(start: 18.0, end: 3.0),
            Pin(size: 69.0, middle: 0.624),
            child: SingleChildScrollView(
              primary: false,
              scrollDirection: Axis.horizontal,
              child: SizedBox(
                width: 888.0,
                height: 69.0,
                child: Stack(
                  children: <Widget>[
                    Padding(
                      padding: EdgeInsets.fromLTRB(0.0, 0.0, -481.0, 0.0),
                      child: SingleChildScrollView(
                        primary: false,
                        child: Wrap(
                          alignment: WrapAlignment.center,
                          spacing: 20,
                          runSpacing: 20,
                          children: [{}].map((itemData) {
                            return SizedBox(
                              width: 888.0,
                              height: 69.0,
                              child: Stack(
                                children: <Widget>[
                                  Pinned.fromPins(
                                    Pin(size: 137.0, start: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        image: DecorationImage(
                                          image: const AssetImage(''),
                                          fit: BoxFit.fill,
                                        ),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 137.0, middle: 0.1997),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        image: DecorationImage(
                                          image: const AssetImage(''),
                                          fit: BoxFit.fill,
                                        ),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 137.0, middle: 0.3995),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        image: DecorationImage(
                                          image: const AssetImage(''),
                                          fit: BoxFit.fill,
                                        ),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 137.0, middle: 0.5992),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        image: DecorationImage(
                                          image: const AssetImage(''),
                                          fit: BoxFit.fill,
                                        ),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 137.0, middle: 0.7989),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        image: DecorationImage(
                                          image: const AssetImage(''),
                                          fit: BoxFit.fill,
                                        ),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 138.0, end: 0.0),
                                    Pin(start: 0.0, end: 0.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        image: DecorationImage(
                                          image: const AssetImage(''),
                                          fit: BoxFit.fill,
                                        ),
                                      ),
                                    ),
                                  ),
                                ],
                              ),
                            );
                          }).toList(),
                        ),
                      ),
                    ),
                  ],
                ),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(size: 89.0, start: 18.0),
            Pin(size: 24.0, start: 1168.0),
            child: Text(
              'Sponsors',
              style: TextStyle(
                fontFamily: 'Montserrat-SemiBold',
                fontSize: 20,
                color: const Color(0xff08080a),
                letterSpacing: -0.8,
                height: 1.85,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(size: 82.0, start: 337.0),
            Pin(size: 18.0, start: 1172.0),
            child: Text(
              'View Offers',
              style: TextStyle(
                fontFamily: 'Montserrat-SemiBold',
                fontSize: 14,
                color: const Color(0xff535353),
                letterSpacing: -0.14,
                height: 2.2857142857142856,
              ),
              textHeightBehavior:
                  TextHeightBehavior(applyHeightToFirstAscent: false),
              softWrap: false,
            ),
          ),
          Pinned.fromPins(
            Pin(start: 0.0, end: 0.0),
            Pin(size: 266.0, start: 252.0),
            child: SingleChildScrollView(
              primary: false,
              scrollDirection: Axis.horizontal,
              child: SizedBox(
                width: 763.0,
                height: 266.0,
                child: Stack(
                  children: <Widget>[
                    Padding(
                      padding: EdgeInsets.fromLTRB(0.0, 0.0, -335.0, 0.0),
                      child: SingleChildScrollView(
                        primary: false,
                        child: Wrap(
                          alignment: WrapAlignment.center,
                          spacing: 0,
                          runSpacing: 20,
                          children: [{}, {}, {}, {}, {}].map((itemData) {
                            return SizedBox(
                              width: 172.0,
                              height: 209.0,
                              child: Stack(
                                children: <Widget>[
                                  Container(
                                    decoration: BoxDecoration(
                                      color: const Color(0xffffffff),
                                      borderRadius: BorderRadius.circular(18.0),
                                      boxShadow: [
                                        BoxShadow(
                                          color: const Color(0x0a000000),
                                          offset: Offset(0, 3),
                                          blurRadius: 20,
                                        ),
                                      ],
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(start: 8.0, end: 8.0),
                                    Pin(size: 134.0, start: 8.0),
                                    child: Stack(
                                      children: <Widget>[
                                        Container(
                                          decoration: BoxDecoration(
                                            image: DecorationImage(
                                              image: const AssetImage(''),
                                              fit: BoxFit.fill,
                                            ),
                                          ),
                                          margin: EdgeInsets.fromLTRB(
                                              -46.0, -23.0, -58.0, -9.0),
                                        ),
                                        Container(
                                          decoration: BoxDecoration(
                                            color: const Color(0xffffffff),
                                            borderRadius: BorderRadius.only(
                                              topLeft: Radius.circular(13.0),
                                              topRight: Radius.circular(13.0),
                                            ),
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 80.0, middle: 0.5),
                                    Pin(size: 17.0, start: 8.0),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        color: const Color(0xffffffff),
                                        borderRadius: BorderRadius.only(
                                          bottomRight: Radius.circular(7.0),
                                          bottomLeft: Radius.circular(7.0),
                                        ),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 108.0, start: 8.0),
                                    Pin(size: 20.0, middle: 0.7831),
                                    child: Text(
                                      'Vie Swim Week',
                                      style: TextStyle(
                                        fontFamily: 'Poppins',
                                        fontSize: 14,
                                        color: const Color(0xff292929),
                                        fontWeight: FontWeight.w600,
                                        height: 3.5,
                                      ),
                                      textHeightBehavior: TextHeightBehavior(
                                          applyHeightToFirstAscent: false),
                                      softWrap: false,
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 54.0, start: 8.0),
                                    Pin(size: 12.0, end: 21.5),
                                    child: Stack(
                                      children: <Widget>[
                                        Pinned.fromPins(
                                          Pin(size: 6.8, start: 0.0),
                                          Pin(start: 0.2, end: 1.6),
                                          child: Stack(
                                            children: <Widget>[
                                              SizedBox.expand(
                                                  child: SvgPicture.string(
                                                _svg_e8mr51,
                                                allowDrawingOutsideViewBox:
                                                    true,
                                                fit: BoxFit.fill,
                                              )),
                                            ],
                                          ),
                                        ),
                                        Pinned.fromPins(
                                          Pin(size: 42.0, end: 0.0),
                                          Pin(start: 0.0, end: 0.0),
                                          child: Text(
                                            'Colombo',
                                            style: TextStyle(
                                              fontFamily: 'Poppins',
                                              fontSize: 9,
                                              color: const Color(0xff9d9d9d),
                                              fontWeight: FontWeight.w300,
                                              height: 5.333333333333333,
                                            ),
                                            textHeightBehavior:
                                                TextHeightBehavior(
                                                    applyHeightToFirstAscent:
                                                        false),
                                            softWrap: false,
                                          ),
                                        ),
                                      ],
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 17.0, middle: 0.5356),
                                    Pin(size: 17.0, end: 19.5),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        color: const Color(0xffe4be63),
                                        borderRadius: BorderRadius.all(
                                            Radius.elliptical(9999.0, 9999.0)),
                                        border: Border.all(
                                            width: 1.0,
                                            color: const Color(0xffffffff)),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 17.0, middle: 0.5679),
                                    Pin(size: 17.0, end: 19.5),
                                    child: Container(
                                      decoration: BoxDecoration(
                                        color: const Color(0xffe4be63),
                                        borderRadius: BorderRadius.all(
                                            Radius.elliptical(9999.0, 9999.0)),
                                        border: Border.all(
                                            width: 1.0,
                                            color: const Color(0xffffffff)),
                                      ),
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 51.0, end: 13.0),
                                    Pin(size: 12.0, end: 21.5),
                                    child: Text(
                                      '250+ Going',
                                      style: TextStyle(
                                        fontFamily: 'Poppins',
                                        fontSize: 9,
                                        color: const Color(0xff9d9d9d),
                                        fontWeight: FontWeight.w300,
                                        height: 5.333333333333333,
                                      ),
                                      textHeightBehavior: TextHeightBehavior(
                                          applyHeightToFirstAscent: false),
                                      softWrap: false,
                                    ),
                                  ),
                                  Pinned.fromPins(
                                    Pin(size: 70.0, middle: 0.5),
                                    Pin(size: 12.0, start: 10.0),
                                    child: Stack(
                                      children: <Widget>[
                                        SizedBox.expand(
                                            child: Text(
                                          'November 2022',
                                          style: TextStyle(
                                            fontFamily: 'Poppins',
                                            fontSize: 9,
                                            color: const Color(0xff292929),
                                            fontWeight: FontWeight.w300,
                                            height: 3.5555555555555554,
                                          ),
                                          textHeightBehavior:
                                              TextHeightBehavior(
                                                  applyHeightToFirstAscent:
                                                      false),
                                          softWrap: false,
                                        )),
                                      ],
                                    ),
                                  ),
                                ],
                              ),
                            );
                          }).toList(),
                        ),
                      ),
                    ),
                  ],
                ),
              ),
            ),
          ),
          Pinned.fromPins(
            Pin(start: 40.0, end: 40.0),
            Pin(size: 64.0, middle: 0.7267),
            child: Stack(
              children: <Widget>[
                Container(
                  decoration: BoxDecoration(
                    color: const Color(0xffffffff),
                    borderRadius: BorderRadius.circular(32.0),
                    boxShadow: [
                      BoxShadow(
                        color: const Color(0x1f000000),
                        offset: Offset(0, 3),
                        blurRadius: 20,
                      ),
                    ],
                  ),
                ),
                Pinned.fromPins(
                  Pin(size: 21.6, start: 39.0),
                  Pin(size: 24.0, middle: 0.5),
                  child: Stack(
                    children: <Widget>[
                      SizedBox.expand(
                          child: SvgPicture.string(
                        _svg_ltvpjk,
                        allowDrawingOutsideViewBox: true,
                        fit: BoxFit.fill,
                      )),
                    ],
                  ),
                ),
                Align(
                  alignment: Alignment(0.006, 0.0),
                  child: SizedBox(
                    width: 20.0,
                    height: 24.0,
                    child: Stack(
                      children: <Widget>[
                        SizedBox.expand(
                            child: SvgPicture.string(
                          _svg_d9ui4l,
                          allowDrawingOutsideViewBox: true,
                          fit: BoxFit.fill,
                        )),
                      ],
                    ),
                  ),
                ),
                Pinned.fromPins(
                  Pin(size: 24.0, end: 35.0),
                  Pin(size: 24.0, middle: 0.5),
                  child: Stack(
                    children: <Widget>[
                      SizedBox.expand(
                          child: SvgPicture.string(
                        _svg_ales68,
                        allowDrawingOutsideViewBox: true,
                        fit: BoxFit.fill,
                      )),
                      Align(
                        alignment: Alignment(0.0, -0.475),
                        child: SizedBox(
                          width: 6.0,
                          height: 6.0,
                          child: SvgPicture.string(
                            _svg_qgpgd8,
                            allowDrawingOutsideViewBox: true,
                          ),
                        ),
                      ),
                    ],
                  ),
                ),
                Align(
                  alignment: Alignment(0.392, 0.0),
                  child: SizedBox(
                    width: 26.0,
                    height: 24.0,
                    child: Stack(
                      children: <Widget>[
                        SizedBox.expand(
                            child: SvgPicture.string(
                          _svg_gg2f,
                          allowDrawingOutsideViewBox: true,
                          fit: BoxFit.fill,
                        )),
                      ],
                    ),
                  ),
                ),
                Align(
                  alignment: Alignment(-0.379, 0.0),
                  child: SizedBox(
                    width: 26.0,
                    height: 26.0,
                    child: Stack(
                      children: <Widget>[
                        SizedBox.expand(
                            child: SvgPicture.string(
                          _svg_pc77xu,
                          allowDrawingOutsideViewBox: true,
                          fit: BoxFit.fill,
                        )),
                        Padding(
                          padding: EdgeInsets.all(2.2),
                          child: SizedBox.expand(
                              child: SvgPicture.string(
                            _svg_icfiaz,
                            allowDrawingOutsideViewBox: true,
                            fit: BoxFit.fill,
                          )),
                        ),
                      ],
                    ),
                  ),
                ),
              ],
            ),
          ),
        ],
      ),
    );
  }
}

const String _svg_e8mr51 =
    '<svg viewBox="0.0 0.0 6.8 10.2" ><path  d="M 3.404032468795776 0 C 1.524098992347717 0 0 1.423736572265625 0 3.187025308609009 C 0 6.236187934875488 3.404032468795776 10.21209812164307 3.404032468795776 10.21209812164307 C 3.404032468795776 10.21209812164307 6.808064937591553 6.235336780548096 6.808064937591553 3.187025308609009 C 6.808064937591553 1.424587726593018 5.283966541290283 0 3.404032468795776 0 Z M 3.404032468795776 4.950314044952393 C 2.91657543182373 4.950314044952393 2.449088096618652 4.76819896697998 2.104146003723145 4.444815635681152 C 1.759204268455505 4.121432304382324 1.565855145454407 3.683163404464722 1.565855145454407 3.226171731948853 C 1.565855145454407 2.768329381942749 1.759204268455505 2.330060243606567 2.104146003723145 2.006677150726318 C 2.449088096618652 1.683294057846069 2.91657543182373 1.501178383827209 3.404032468795776 1.501178383827209 C 3.891490459442139 1.501178383827209 4.358977317810059 1.683294057846069 4.703919410705566 2.006677150726318 C 5.048861026763916 2.330060243606567 5.242210388183594 2.768329381942749 5.242210388183594 3.226171731948853 C 5.242210388183594 3.683163404464722 5.048861026763916 4.121432304382324 4.703919410705566 4.444815635681152 C 4.358977317810059 4.76819896697998 3.891490459442139 4.950314044952393 3.404032468795776 4.950314044952393 Z" fill="#373636" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_ipnvfy =
    '<svg viewBox="0.0 0.0 24.5 11.5" ><path  d="M 3.589200019836426 11.50020027160645 C 2.358000040054321 11.50020027160645 1.897199988365173 11.37419986724854 1.432800054550171 11.12580013275146 C 0.9747000336647034 10.88100051879883 0.6191999912261963 10.52550029754639 0.3744000196456909 10.0673999786377 C 0.1260000020265579 9.604800224304199 0 9.144000053405762 0 7.91100025177002 L 0 3.589200019836426 C 0 2.356199979782104 0.1260000020265579 1.895400047302246 0.3744000196456909 1.432800054550171 C 0.6191999912261963 0.9747000336647034 0.9747000336647034 0.6191999912261963 1.432800054550171 0.3744000196456909 C 1.895400047302246 0.1260000020265579 2.356199979782104 0 3.589200019836426 0 L 18.410400390625 0 C 19.64430046081543 0 20.10510063171387 0.1260000020265579 20.56770133972168 0.3744000196456909 C 21.02580070495605 0.6191999912261963 21.38129997253418 0.9747000336647034 21.62610054016113 1.432800054550171 C 21.87360000610352 1.896300077438354 21.99960136413574 2.357100009918213 21.99960136413574 3.589200019836426 L 21.99960136413574 7.91100025177002 C 21.99960136413574 9.143099784851074 21.87360000610352 9.603899955749512 21.62610054016113 10.0673999786377 C 21.38129997253418 10.52550029754639 21.02580070495605 10.88100051879883 20.56770133972168 11.12580013275146 C 20.10330009460449 11.37419986724854 19.64159965515137 11.50020027160645 18.410400390625 11.50020027160645 L 3.589200019836426 11.50020027160645 Z M 1.904399991035461 1.255500078201294 C 1.620000004768372 1.407600045204163 1.407600045204163 1.620000004768372 1.255500078201294 1.904399991035461 C 1.057500004768372 2.275200128555298 0.9999000430107117 2.653200149536133 0.9999000430107117 3.589200019836426 L 0.9999000430107117 7.91100025177002 C 0.9999000430107117 8.847000122070312 1.057500004768372 9.225000381469727 1.255500078201294 9.595800399780273 C 1.407600045204163 9.881100654602051 1.620000004768372 10.09259986877441 1.904399991035461 10.24380016326904 C 2.275200128555298 10.44270038604736 2.653200149536133 10.50030040740967 3.589200019836426 10.50030040740967 L 18.410400390625 10.50030040740967 C 19.34729957580566 10.50030040740967 19.72529983520508 10.44270038604736 20.09609985351562 10.24380016326904 C 20.37689971923828 10.0935001373291 20.59560012817383 9.875699996948242 20.74410057067871 9.595800399780273 C 20.94210052490234 9.225000381469727 20.99970054626465 8.847000122070312 20.99970054626465 7.91100025177002 L 20.99970054626465 3.589200019836426 C 20.99970054626465 2.653200149536133 20.94210052490234 2.275200128555298 20.74410057067871 1.904399991035461 C 20.59560012817383 1.624500036239624 20.37779998779297 1.406700015068054 20.09609985351562 1.255500078201294 C 19.72529983520508 1.057500004768372 19.34729957580566 0.9999000430107117 18.410400390625 0.9999000430107117 L 3.589200019836426 0.9999000430107117 C 2.653200149536133 0.9999000430107117 2.275200128555298 1.057500004768372 1.904399991035461 1.255500078201294 Z M 23.00040054321289 3.690000057220459 C 23.00040054321289 3.690000057220459 24.49979972839355 4.453200340270996 24.49979972839355 5.689800262451172 C 24.49979972839355 6.926400184631348 23.00040054321289 7.689599990844727 23.00040054321289 7.689599990844727 L 23.00040054321289 3.690000057220459 Z" fill="#323f4b" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_hpzmte =
    '<svg viewBox="0.0 1.0 17.1 10.7" ><path transform="translate(0.0, 1.0)" d="M 15.30000019073486 10.70009994506836 C 14.63759994506836 10.70009994506836 14.10030078887939 10.16279983520508 14.10030078887939 9.500400543212891 L 14.10030078887939 1.199699997901917 C 14.10030078887939 0.5372999906539917 14.63759994506836 0 15.30000019073486 0 L 15.90030002593994 0 C 16.56270027160645 0 17.10000038146973 0.5372999906539917 17.10000038146973 1.199699997901917 L 17.10000038146973 9.500400543212891 C 17.10000038146973 10.16279983520508 16.56270027160645 10.70009994506836 15.90030002593994 10.70009994506836 L 15.30000019073486 10.70009994506836 Z M 10.60020065307617 10.70009994506836 C 9.93690013885498 10.70009994506836 9.399600028991699 10.16279983520508 9.399600028991699 9.499500274658203 L 9.399600028991699 3.600000143051147 C 9.399600028991699 2.937600135803223 9.93690013885498 2.400300025939941 10.60020065307617 2.400300025939941 L 11.19960021972656 2.400300025939941 C 11.86290073394775 2.400300025939941 12.40019989013672 2.937600135803223 12.40019989013672 3.600000143051147 L 12.40019989013672 9.499500274658203 C 12.40019989013672 10.16279983520508 11.86290073394775 10.70009994506836 11.19960021972656 10.70009994506836 L 10.60020065307617 10.70009994506836 Z M 6.00029993057251 10.70009994506836 C 5.336999893188477 10.70009994506836 4.799700260162354 10.16279983520508 4.799700260162354 9.500400543212891 L 4.799700260162354 5.900400161743164 C 4.799700260162354 5.237100124359131 5.336999893188477 4.69980001449585 6.00029993057251 4.69980001449585 L 6.599699974060059 4.69980001449585 C 7.263000011444092 4.69980001449585 7.800300121307373 5.237100124359131 7.800300121307373 5.900400161743164 L 7.800300121307373 9.500400543212891 C 7.800300121307373 10.16279983520508 7.263000011444092 10.70009994506836 6.599699974060059 10.70009994506836 L 6.00029993057251 10.70009994506836 Z M 1.199699997901917 10.70009994506836 C 0.5372999906539917 10.70009994506836 0 10.16279983520508 0 9.500400543212891 L 0 7.900200366973877 C 0 7.236900329589844 0.5372999906539917 6.699600219726562 1.199699997901917 6.699600219726562 L 1.800000071525574 6.699600219726562 C 2.462399959564209 6.699600219726562 2.99970006942749 7.236900329589844 2.99970006942749 7.900200366973877 L 2.99970006942749 9.500400543212891 C 2.99970006942749 10.16279983520508 2.462399959564209 10.70009994506836 1.800000071525574 10.70009994506836 L 1.199699997901917 10.70009994506836 Z" fill="#323f4b" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_vql4yc =
    '<svg viewBox="22.1 0.8 15.4 11.1" ><path transform="translate(22.1, 0.8)" d="M 7.700416564941406 11.05716991424561 C 7.617156505584717 11.05716991424561 7.535806655883789 11.02304935455322 7.477216243743896 10.96356964111328 L 5.462116241455078 8.931369781494141 C 5.400266647338867 8.870419502258301 5.365816593170166 8.785449981689453 5.367616653442383 8.698269844055176 C 5.369406223297119 8.611849784851074 5.407456398010254 8.528860092163086 5.472016334533691 8.470569610595703 C 6.09403657913208 7.944310188293457 6.885416507720947 7.654500007629395 7.700416564941406 7.654500007629395 C 8.515416145324707 7.654500007629395 9.306806564331055 7.944320201873779 9.928816795349121 8.470569610595703 C 9.993746757507324 8.529190063476562 10.03049659729004 8.610049247741699 10.03231620788574 8.698269844055176 C 10.03411674499512 8.785449981689453 9.999666213989258 8.870409965515137 9.937816619873047 8.931369781494141 L 7.923616409301758 10.96356964111328 C 7.865026473999023 11.02304935455322 7.783676624298096 11.05716991424561 7.700416564941406 11.05716991424561 Z M 11.23713684082031 7.490050315856934 C 11.15660667419434 7.490050315856934 11.0803165435791 7.45952033996582 11.02231597900391 7.404070377349854 C 10.10989665985107 6.578210353851318 8.930146217346191 6.123370170593262 7.700416564941406 6.123370170593262 C 6.471526622772217 6.124269962310791 5.292746543884277 6.579100131988525 4.381216526031494 7.404070377349854 C 4.322786331176758 7.459510326385498 4.246386528015137 7.490050315856934 4.166086673736572 7.490050315856934 C 4.082406520843506 7.490060329437256 4.003786563873291 7.457290172576904 3.944716691970825 7.397770404815674 L 2.78011679649353 6.221470355987549 C 2.71916675567627 6.160520076751709 2.684726715087891 6.07588005065918 2.685616731643677 5.989270210266113 C 2.686506748199463 5.903069972991943 2.722596645355225 5.819090366363525 2.784616708755493 5.758870124816895 C 4.124846458435059 4.51222038269043 5.871116638183594 3.825670003890991 7.701766490936279 3.825670003890991 C 9.532416343688965 3.825670003890991 11.27869606018066 4.512230396270752 12.61891651153564 5.758870124816895 C 12.68094635009766 5.819100379943848 12.71702671051025 5.903069972991943 12.71791648864746 5.989270210266113 C 12.71880626678467 6.07588005065918 12.68436622619629 6.160520076751709 12.62341690063477 6.221470355987549 L 11.45881652832031 7.397770404815674 C 11.39975643157959 7.457280158996582 11.32102680206299 7.490050315856934 11.23713684082031 7.490050315856934 Z M 13.92011642456055 4.782490253448486 C 13.83861637115479 4.782490253448486 13.76165676116943 4.751120090484619 13.70341682434082 4.694159984588623 C 12.07592678070068 3.148059844970703 9.944016456604004 2.296570062637329 7.700416564941406 2.296570062637329 C 5.454956531524658 2.296570062637329 3.323046684265137 3.148059844970703 1.697416543960571 4.694169998168945 C 1.638736605644226 4.751120090484619 1.561486601829529 4.782490253448486 1.479926586151123 4.782490253448486 C 1.396516561508179 4.782490253448486 1.318416595458984 4.749850273132324 1.260016560554504 4.69057035446167 L 0.09361660480499268 3.514270067214966 C 0.03324660658836365 3.452990055084229 -0.0008633918478153646 3.369019985198975 1.66081572388066e-05 3.283869981765747 C 0.0009166081435978413 3.196759939193726 0.03543660789728165 3.115249872207642 0.09721660614013672 3.054369926452637 C 2.152616739273071 1.084729909896851 4.852656364440918 0 7.699966430664062 0 C 10.54727649688721 0 13.24731636047363 1.084729909896851 15.30271625518799 3.054369926452637 C 15.36539649963379 3.116139888763428 15.39991664886475 3.197639942169189 15.39991664886475 3.283869981765747 C 15.40081691741943 3.370599985122681 15.36725616455078 3.452419996261597 15.30541610717773 3.514270067214966 L 14.13991641998291 4.69057035446167 C 14.08151626586914 4.749850273132324 14.00345611572266 4.782490253448486 13.92011642456055 4.782490253448486 Z" fill="#323f4b" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_he76qv =
    '<svg viewBox="0.0 6.3 18.5 16.0" ><path transform="translate(0.0, 1.46)" d="M 15.44709396362305 14.70765972137451 L 15.44709396362305 11.08505439758301 C 15.44709396362305 7.658721923828125 12.66937255859375 4.881000518798828 9.243040084838867 4.881000518798828 C 5.816707134246826 4.881000518798828 3.038985729217529 7.658721923828125 3.038985729217529 11.08505439758301 L 3.038985729217529 14.87273693084717 L 0 20.87012100219727 L 3.038985729217529 20.87012100219727 L 15.44709396362305 20.87012100219727 L 18.48608016967773 20.87012100219727 L 15.44709396362305 14.70765972137451 Z" fill="#373636" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_oq9a97 =
    '<svg viewBox="7.0 25.2 4.5 2.8" ><path transform="translate(1.61, 5.8)" d="M 7.629693984985352 22.19201469421387 L 7.629693984985352 22.19201469421387 C 6.388362884521484 22.19201469421387 5.381000518798828 21.18595123291016 5.381000518798828 19.94332122802734 L 5.381000518798828 19.36100006103516 L 9.878386497497559 19.36100006103516 L 9.878386497497559 19.94332122802734 C 9.878386497497559 21.18595123291016 8.871025085449219 22.19201469421387 7.629693984985352 22.19201469421387" fill="#373636" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_h0g8bd =
    '<svg viewBox="9.2 0.0 12.7 12.7" ><path transform="translate(2.13, 0.0)" d="M 19.80216979980469 6.3457350730896 C 19.80216979980469 9.850057601928711 16.96205711364746 12.69016933441162 13.45773601531982 12.69016933441162 C 9.953413009643555 12.69016933441162 7.112000465393066 9.850057601928711 7.112000465393066 6.3457350730896 C 7.112000465393066 2.841412782669067 9.953413009643555 0 13.45773601531982 0 C 16.96205711364746 0 19.80216979980469 2.841412782669067 19.80216979980469 6.3457350730896" fill="#ff0000" stroke="none" stroke-width="1" stroke-miterlimit="10" stroke-linecap="butt" /></svg>';
const String _svg_cuxc0u =
    '<svg viewBox="0.0 2.5 5.7 1.0" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 5.73, 2.45)" d="M 0 5.727272510528564 L 0 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_t8k8zx =
    '<svg viewBox="9.0 2.5 5.7 1.0" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 14.73, 2.45)" d="M 0 5.727272510528564 L 0 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_ppjs0r =
    '<svg viewBox="0.0 9.0 7.4 1.0" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 7.36, 9.0)" d="M 0 7.363636493682861 L 0 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_hia3z =
    '<svg viewBox="10.6 9.0 4.1 1.0" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 14.73, 9.0)" d="M 0 4.090909004211426 L 0 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_r226f3 =
    '<svg viewBox="0.0 15.5 4.1 1.0" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 4.09, 15.55)" d="M 0 4.090909004211426 L 0 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_u7ts9w =
    '<svg viewBox="7.4 15.5 7.4 1.0" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 14.73, 15.55)" d="M 0 7.363636493682861 L 0 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_ighjoy =
    '<svg viewBox="5.7 0.0 1.0 4.9" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 5.73, 0.0)" d="M 0 0 L 4.909090995788574 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_d251xk =
    '<svg viewBox="10.6 6.5 1.0 4.9" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 10.64, 6.55)" d="M 0 0 L 4.909090995788574 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_c8w2ub =
    '<svg viewBox="4.1 13.1 1.0 4.9" ><path transform="matrix(0.0, 1.0, -1.0, 0.0, 4.09, 13.09)" d="M 0 0 L 4.909090995788574 0" fill="none" stroke="#373636" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" /></svg>';
const String _svg_ltvpjk =
    '<svg viewBox="53.0 753.0 21.6 24.0" ><path transform="translate(50.0, 751.02)" d="M 23.4000186920166 9.196146965026855 L 23.4000186920166 9.196146965026855 L 16.20001983642578 2.891690731048584 C 14.83345699310303 1.670860171318054 12.76658248901367 1.670859694480896 11.4000186920166 2.891690254211426 L 4.200019359588623 9.196146965026855 C 3.428952932357788 9.884927749633789 2.991796493530273 10.87172031402588 3.000019550323486 11.9049072265625 L 3.000019073486328 22.38036918640137 C 3.000019073486328 24.3662166595459 4.611794471740723 25.97606658935547 6.600019454956055 25.97606658935547 L 21.00001907348633 25.97606658935547 C 22.98824501037598 25.97606658935547 24.60001945495605 24.3662166595459 24.60001945495605 22.38036918640137 L 24.60001945495605 11.89292144775391 C 24.60479164123535 10.86390590667725 24.16793632507324 9.882155418395996 23.4000186920166 9.196147918701172 Z M 16.20001983642578 23.57893371582031 L 11.4000186920166 23.57893371582031 L 11.4000186920166 17.58610725402832 C 11.4000186920166 16.92415618896484 11.93727779388428 16.38754081726074 12.60001945495605 16.38754081726074 L 15.00001907348633 16.38754081726074 C 15.66276168823242 16.38754081726074 16.20001983642578 16.92415809631348 16.20001983642578 17.58610725402832 L 16.20001983642578 23.57893371582031 Z M 22.20001983642578 22.38036918640137 C 22.20001983642578 23.04231643676758 21.66275978088379 23.57893371582031 21.00001907348633 23.57893371582031 L 18.60001945495605 23.57893371582031 L 18.60001945495605 17.58610725402832 C 18.60001945495605 15.60025882720947 16.98824501037598 13.99040985107422 15.00001907348633 13.99040985107422 L 12.60001945495605 13.99040985107422 C 10.61179351806641 13.99040985107422 9.000018119812012 15.60025978088379 9.000019073486328 17.58610725402832 L 9.000019073486328 23.57893371582031 L 6.600019454956055 23.57893371582031 C 5.937277793884277 23.57893371582031 5.40001916885376 23.04231643676758 5.40001916885376 22.38036918640137 L 5.40001916885376 11.89292144775391 C 5.400455951690674 11.548659324646 5.549078941345215 11.22120761871338 5.808019638061523 10.99399662017822 L 13.00801944732666 4.701525211334229 C 13.46102714538574 4.304025173187256 14.13901138305664 4.304025173187256 14.59201908111572 4.701525211334229 L 21.79201889038086 10.99399662017822 C 22.05096054077148 11.2212085723877 22.19958305358887 11.54866123199463 22.20001983642578 11.89292144775391 L 22.20001983642578 22.38036918640137 Z" fill="#3c94ff" stroke="none" stroke-width="1" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_d9ui4l =
    '<svg viewBox="136.0 753.0 20.0 24.0" ><path transform="translate(134.0, 751.0)" d="M 19 4.400000095367432 L 17 4.400000095367432 L 17 3.200000047683716 C 17 2.537258386611938 16.55228424072266 2 16 2 C 15.44771480560303 2 15 2.537258386611938 15 3.200000047683716 L 15 4.400000095367432 L 9 4.400000095367432 L 9 3.200000047683716 C 9 2.537258386611938 8.552284240722656 2 8 2 C 7.447715282440186 2 7 2.537258386611938 7 3.200000047683716 L 7 4.400000095367432 L 5 4.400000095367432 C 3.343145608901978 4.40000057220459 1.999999523162842 6.011776447296143 2 8.000001907348633 L 2 22.40000152587891 C 2 24.38822746276855 3.343145847320557 26.00000190734863 5 26.00000190734863 L 19 26.00000190734863 C 20.6568546295166 26.00000190734863 22 24.38822746276855 22 22.40000152587891 L 22 8.000000953674316 C 22 6.011775970458984 20.6568546295166 4.40000057220459 19 4.40000057220459 Z M 20 22.40000152587891 C 20 23.06274223327637 19.55228424072266 23.60000038146973 19 23.60000038146973 L 5 23.60000038146973 C 4.447715282440186 23.60000038146973 4 23.06274223327637 4 22.40000152587891 L 4 14.00000095367432 L 20 14.00000095367432 L 20 22.40000152587891 Z M 20 11.60000038146973 L 4 11.60000038146973 L 4 8.000000953674316 C 4 7.337258338928223 4.447715282440186 6.800000190734863 5 6.800000190734863 L 7 6.800000190734863 L 7 8.000000953674316 C 7 8.662742614746094 7.447715282440186 9.200000762939453 8 9.200000762939453 C 8.552285194396973 9.200000762939453 9 8.662742614746094 9 8.000000953674316 L 9 6.800000190734863 L 15 6.800000190734863 L 15 8.000000953674316 C 15 8.662742614746094 15.44771575927734 9.200000762939453 16 9.200000762939453 C 16.55228424072266 9.200000762939453 17 8.662742614746094 17 8.000000953674316 L 17 6.800000190734863 L 19 6.800000190734863 C 19.55228424072266 6.800000190734863 20 7.337258338928223 20 8.000000953674316 L 20 11.60000038146973 Z" fill="#3c94ff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_ales68 =
    '<svg viewBox="0.5 0.5 24.0 24.0" ><path  d="M 12.5 0.5 C 5.883068084716797 0.5 0.5 5.883068084716797 0.5 12.5 C 0.5 19.11693382263184 5.883068084716797 24.5 12.5 24.5 C 19.11693382263184 24.5 24.5 19.11695671081543 24.5 12.5 C 24.5 5.883045196533203 19.11695671081543 0.5 12.5 0.5 Z M 12.5 22.17741966247559 C 9.947863578796387 22.17741966247559 7.63019847869873 21.17707633972168 5.899318695068359 19.5571460723877 C 6.211806774139404 18.97640800476074 6.635848045349121 18.44877815246582 7.158041000366211 17.98900985717773 C 8.489676475524902 16.81225776672363 10.41741943359375 16.09225654602051 12.5 16.09225654602051 C 14.96192741394043 16.09225654602051 17.20711326599121 17.09870147705078 18.5 18.67803955078125 C 18.73489761352539 18.95437240600586 18.92555999755859 19.2542896270752 19.09307289123535 19.5640811920166 C 17.36294174194336 21.17987251281738 15.04841327667236 22.17741966247559 12.5 22.17741966247559 Z M 20.66410064697266 17.67409133911133 C 18.99432373046875 15.28477478027344 15.93703269958496 13.76967620849609 12.5 13.76967620849609 C 9.059274673461914 13.76967620849609 5.999777793884277 15.28791618347168 4.342322826385498 17.68463325500488 C 3.385125637054443 16.18392181396484 2.822580814361572 14.40815448760986 2.822580814361572 12.5 C 2.822580814361572 7.163808822631836 7.163808822631836 2.822580814361572 12.5 2.822580814361572 C 17.83619117736816 2.822580814361572 22.17741966247559 7.163808822631836 22.17741966247559 12.5 C 22.17741966247559 14.40374183654785 21.61721038818359 16.17556190490723 20.66410064697266 17.67409133911133 Z" fill="#3c94ff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_qgpgd8 =
    '<svg viewBox="9.3 5.1 6.4 6.4" ><path transform="translate(-1.74, -0.91)" d="M 14.24016380310059 6.025390148162842 C 12.46743392944336 6.025390148162842 11.025390625 7.467433929443359 11.025390625 9.240163803100586 C 11.025390625 11.0128927230835 12.46740818023682 12.45493030548096 14.24016380310059 12.45493030548096 C 16.01292037963867 12.45493030548096 17.45493125915527 11.01291942596436 17.45493125915527 9.240163803100586 C 17.45493125915527 7.467408180236816 16.01292037963867 6.025390148162842 14.24016380310059 6.025390148162842 Z M 14.24016380310059 10.51622200012207 C 13.53649616241455 10.51622200012207 12.96409893035889 9.94382381439209 12.96409893035889 9.240157127380371 C 12.96409893035889 8.536489486694336 13.53647708892822 7.964098930358887 14.24016380310059 7.964098930358887 C 14.94385051727295 7.964098930358887 15.51622200012207 8.536496162414551 15.51622200012207 9.240163803100586 C 15.51622200012207 9.943831443786621 14.94385051727295 10.51622200012207 14.24016380310059 10.51622200012207 Z" fill="#3c94ff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_gg2f =
    '<svg viewBox="216.4 753.1 26.3 24.0" ><path transform="translate(214.62, 750.09)" d="M 20.31559562683105 3.000000476837158 C 18.27724647521973 3.001794576644897 16.32303619384766 3.812975883483887 14.88260269165039 5.255206108093262 C 11.88212299346924 2.250829219818115 7.014224529266357 2.247960567474365 4.010027408599854 5.248620510101318 C 1.005829811096191 8.249277114868164 1.002961158752441 13.11717700958252 4.003621101379395 16.12137603759766 L 14.88260078430176 26.99999809265137 L 25.76140403747559 16.12119674682617 C 27.95900917053223 13.92085456848145 28.61464309692383 10.61330604553223 27.42254447937012 7.741048336029053 C 26.23044395446777 4.868790626525879 23.4254150390625 2.997552871704102 20.31559562683105 3.000000476837158 Z M 23.94186401367188 14.31446838378906 L 14.88260078430176 23.37373161315918 L 5.823338985443115 14.31446838378906 C 3.820606708526611 12.31173419952393 3.82060718536377 9.064662933349609 5.823339462280273 7.061931133270264 C 7.826070308685303 5.059200286865234 11.07314109802246 5.059200286865234 13.07587432861328 7.06193208694458 L 14.88260078430176 8.881473541259766 L 16.68932723999023 7.074747085571289 C 18.71912574768066 5.13845682144165 21.91206741333008 5.138458251953125 23.94186401367188 7.074747562408447 C 24.90430641174316 8.036123275756836 25.44509315490723 9.340669631958008 25.44509315490723 10.70101451873779 C 25.44509315490723 12.06136035919189 24.90430641174316 13.36590671539307 23.94186401367188 14.32728099822998 Z" fill="#3c94ff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_pc77xu =
    '<svg viewBox="0.0 0.0 26.0 26.0" ><path  d="M 0 0 L 26 0 L 26 26 L 0 26 L 0 0 Z" fill="none" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
const String _svg_icfiaz =
    '<svg viewBox="2.2 2.2 21.7 21.7" ><path transform="translate(0.17, 0.17)" d="M 12.83333301544189 2 C 6.853333950042725 2 2 6.853333950042725 2 12.83333301544189 C 2 18.81333541870117 6.853333950042725 23.66666793823242 12.83333301544189 23.66666793823242 C 18.81333541870117 23.66666793823242 23.66666793823242 18.81333541870117 23.66666793823242 12.83333301544189 C 23.66666793823242 6.853333473205566 18.81333541870117 2 12.83333301544189 2 Z M 12.83333301544189 21.50000190734863 C 8.05583381652832 21.50000190734863 4.166666984558105 17.6108341217041 4.166666984558105 12.83333301544189 C 4.166666984558105 8.05583381652832 8.05583381652832 4.166666984558105 12.83333301544189 4.166666984558105 C 17.6108341217041 4.166666984558105 21.50000190734863 8.05583381652832 21.50000190734863 12.83333301544189 C 21.50000190734863 17.6108341217041 17.6108341217041 21.50000190734863 12.83333301544189 21.50000190734863 Z M 6.875000476837158 18.79166793823242 L 15.01083374023438 15.01083374023438 L 18.79166793823242 6.875000476837158 L 10.65583324432373 10.65583324432373 L 6.875000476837158 18.79166793823242 Z M 12.83333301544189 11.64166641235352 C 13.49416637420654 11.64166641235352 14.02500057220459 12.17249965667725 14.02500057220459 12.83333301544189 C 14.02500057220459 13.49416732788086 13.49416732788086 14.02500057220459 12.83333301544189 14.02500057220459 C 12.17249965667725 14.02500057220459 11.64166641235352 13.49416732788086 11.64166641235352 12.83333301544189 C 11.64166641235352 12.17249965667725 12.17249965667725 11.64166641235352 12.83333301544189 11.64166641235352 Z" fill="#3c94ff" fill-opacity="0.4" stroke="none" stroke-width="1" stroke-opacity="0.4" stroke-miterlimit="4" stroke-linecap="butt" /></svg>';
