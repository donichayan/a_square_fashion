...
class _UserViewPageState extends State<UserViewPage> {
                      ...
                      if (state is AfterPageLoad) {
                        ...
                        return SizedBox(
                          ...
                          child: eventTile(
                              ...
                              widget.event.venue,
                              date,
                              widget.isSelected ? "Deregister" : "Registered",
                              state),
                        );
                      }
  ...
  Widget _registerLogic(BuildContext context, AfterPageLoad state) {
    if (widget.isSelected) {
      ...
      return ElevatedButton(
        ...
        onPressed: () {
          final date =
              DateFormat('dd-MM-yyyy').format(widget.event.date).toString();
  ...
  Widget eventTile(BuildContext context, String urlimg, String name,
      String place, String date, String buttonText, AfterPageLoad state) {
    return Stack(
      children: <Widget>[
        ...
        /*  Pinned.fromPins(
          ...
          child: InkWell(
            onTap: () {
              if (widget.isSelected) {
                state.userFA.events.removeWhere(
                    (item) => item.eventId == widget.event.referenceID);
                context
                    .read<DetailPageBloc>()
                    .add(UpdateUser(userFA: state.userFA));
              } else {
                state.userFA.events.add(UserEvent(
                    event: widget.event.name,
                    eventId: widget.event.referenceID!));
                context
                    .read<DetailPageBloc>()
                    .add(UpdateUser(userFA: state.userFA));
              }
          ...
          child: Container(
            ...
            child: TextButton(
              ...
              style: ButtonStyle(
                ...
                foregroundColor:
                    MaterialStateProperty.all<Color>(const Color(0xffeed580)),
              ),
              child: Expanded(
                child: Container(
                  decoration: BoxDecoration(
        ...
        Pinned.fromPins(
            Pin(size: 220.0, start: 8.0), Pin(size: 50.0, middle: 0.6890),
            child: RichText(
              text: TextSpan(
        ...
        Pinned.fromPins(
          Pin(size: 17.0, middle: 0.5560),
          Pin(size: 17.0, end: 21.4),
          child: Container(
            decoration: BoxDecoration(
              color: const Color(0xffe4be63),
              borderRadius: BorderRadius.all(Radius.elliptical(9999.0, 9999.0)),
